                                            ...
                                            <div x-show="open" x-transition
                                                ...
                                                @can('users.update-support-group')
                                                    <button wire:click="openSupportGroupModal({{ $user->id }})" @click="open=false"
                                                        class="w-full flex items-center gap-2.5 px-3 py-2 text-left text-teal-600 dark:text-teal-400 hover:bg-gray-50 dark:hover:bg-gray-700/60">
                                                        <svg class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a4 4 0 00-5-3.87M9 20H4v-2a4 4 0 015-3.87m6-4a4 4 0 11-8 0 4 4 0 018 0zm6-4a3 3 0 11-6 0 3 3 0 016 0zM3 8a3 3 0 116 0 3 3 0 01-6 0z"/></svg>
...
<div x-data="{ open: @entangle('showSupportGroupModal') }" x-show="open" x-on:keydown.escape.window="open = false"
    ...
    <div x-show="open" x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0"
        ...
        class="absolute inset-0 bg-gray-500 dark:bg-gray-900 bg-opacity-75"
        x-on:click="open = false">
    </div>
    ...
    <div x-show="open" x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0 scale-95"
        ...
        <div class="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
            <flux:button type="button" wire:click="closeSupportGroupModal">
                Cancel
            </flux:button>
            <flux:button type="button" wire:click="updateSupportGroup" variant="primary">
                Save
            </flux:button>
        </div>
    </div>
...
<div x-data="{ open: @entangle('showBonchatServerModal') }" x-show="open" x-on:keydown.escape.window="open = false"
    ...
    <div x-show="open" x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0 scale-95"
        ...
        <!-- Footer -->
        <div class="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
            <flux:button type="button" wire:click="closeBonchatServerModal">
                Cancel
            </flux:button>
            <flux:button type="button" wire:click="updateBonchatServer" variant="primary">
                Save
            </flux:button>
        </div>
    </div>
