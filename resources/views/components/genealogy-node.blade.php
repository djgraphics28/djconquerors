    ...
    @if($showSuperior && $superior)
        <div class="mb-8 flex flex-col items-center">
            ...
            <div class="relative flex flex-col items-center">
                <!-- Circle Card -->
                <a
                    href="{{ route('genealogy.show', $superior->riscoin_id) }}"
                ...
                >
                    <div class="w-20 h-20 rounded-full border-4 border-purple-500 dark:border-purple-600 overflow-hidden flex items-center justify-center bg-white dark:bg-gray-800 shadow-md">
                        @if($superior->hasMedia('avatar'))
                            <img src="{{ $superior->getFirstMediaUrl('avatar') }}" alt="{{ $superior->name }}" class="w-full h-full object-cover">
                        @else
                ...
                <div class="mt-2 text-center max-w-xs">
                    ...
                    <div class="text-xs text-green-600 dark:text-green-400 font-medium">${{ number_format($superior->invested_amount ?? 0, 2) }}</div>
                    <div class="text-xs dark:text-white mt-1 bg-{{ $superior->is_active ? 'green' : 'red' }}-100 dark:bg-{{ $superior->is_active ? 'green' : 'red' }}-900 text-{{ $superior->is_active ? 'green' : 'red' }}-800 dark:text-{{ $superior->is_active ? 'green' : 'red' }}-100 px-2 py-1 rounded inline-block">
            ...
            >
                <div class="w-24 h-24 rounded-full border-4 {{ $level === 0 ? 'border-blue-500 dark:border-blue-600' : 'border-gray-400 dark:border-gray-600' }} overflow-hidden flex items-center justify-center bg-white dark:bg-gray-800 shadow-md">
                    @if($node->hasMedia('avatar'))
                        <img src="{{ $node->getFirstMediaUrl('avatar') }}" alt="{{ $node->name }}" class="w-full h-full object-cover">
                    @else
